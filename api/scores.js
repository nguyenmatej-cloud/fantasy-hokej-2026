...
function getResult(utcISO, teamA, teamB, fbResults){
  ...
  // SEED_DETAILS first (ručně ověřená data), pak Firebase, pak starý DB
  const seed=SEED_DETAILS[key];
  if(seed)return{h:seed.h,a:seed.a};
  const fb=fbResults?.[key];
  if(fb&&(fb.h||0)+(fb.a||0)>0)return fb;
    ...
    <div>
      {Object.entries(timeGroups).sort().map(([timeUTC,games])=>{
        ...
        return games.map(([t,a,b])=>{
          ...
          const na=COUNTRIES[a]?.n||a;const nb=COUNTRIES[b]?.n||b;
...
class ErrorBoundary extends React.Component{
  ...
  static getDerivedStateFromError(e){return{e:e.message||'Unknown error'};}
  componentDidCatch(e,i){console.error('SquadBuilder crash:',e,i);}
  render(){
    if(this.state.e)return <div style={{padding:20,margin:12,background:'rgba(239,68,68,.1)',border:'1px solid rgba(239,68,68,.3)',borderRadius:12,color:'#fca5a5'}}><b>⚠️ Chyba v záložce Sestavy:</b><br/><code style={{fontSize:11}}>{this.state.e}</code></div>;
...
function App(){
  ...
  useEffect(()=>{
    ...
    // Smaž špatná invertovaná data z TheSportsDB
    db.ref('match_results/2026-05-16_CAN_ITA').once('value').then(s=>{
      const v=s.val();if(v&&v.h===0&&v.a===4)db.ref('match_results/2026-05-16_CAN_ITA').remove();
    }).catch(()=>{});
    db.ref('match_results/2026-05-16_GBR_AUT').once('value').then(s=>{
      const v=s.val();if(v&&v.h===5&&v.a===2)db.ref('match_results/2026-05-16_GBR_AUT').remove();
    }).catch(()=>{});
  ...
  return(
    <div style={{minHeight:'100vh',background:C.bg0,paddingBottom:72}}>
      ...
      <div style={{maxWidth:860,margin:'0 auto',padding:'12px 12px 0',position:'relative',zIndex:1}}>
        ...
        {tab==='sestavy'&&<ErrorBoundary><SquadBuilder myName={myName} rounds={rounds} squadBuilds={squadBuilds} onSquadSave={handleSquadSave} players={players} matchResults={matchResults} isAdmin={ADMINS.includes(myName)} onMatchResult={(k,v)=>db?.ref('match_results/'+k).set(v)} customPlayers={customPlayers}/></ErrorBoundary>}
