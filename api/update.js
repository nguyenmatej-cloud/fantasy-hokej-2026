...
module.exports=async function handler(req,res){
  ...
  const results={day,saved:[],errors:[],debug:[]};
  ...
  try{
    ...
    const tournaments=[...new Set(all.map(e=>e.tournament?.name||e.uniqueTournament?.name||'?'))];
    results.debug=tournaments;
    
    // Flexible filter - catch any IIHF/World Championship event
    const iihf=all.filter(e=>{
      const tn=(e.tournament?.name||e.uniqueTournament?.name||'').toLowerCase();
      const cat=(e.tournament?.category?.name||'').toLowerCase();
      return tn.includes('world')||tn.includes('iihf')||cat.includes('world')||cat.includes('iihf');
    });
    ...
    for(const e of iihf){
      ...
      results.saved.push(`${day}_${found.key}: ${found.h}:${found.a}`);
    }
